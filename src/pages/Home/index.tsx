...
import { AdminSettingsCategory, LandingPageShopByCategory } from "../../utils/types";
...
export const HomePage: React.FC = () => {
  ...
  const [shopByCategoryData, setShopByCategoryData] = useState<LandingPageShopByCategory[]>([]);
  const [adminSettingCardConfigure, setAdminSettingCardConfigure] = useState<AdminSettingsCategory>({
    fontColor: "#004300",
    cardColor: "#d3e2fe",
  });
        ...
        <div className="w-full px-8 responsive-home-page">
          ...
          <div className="relative w-full h-full mx-auto overflow-hidden">
              ...
              {images.map((_, index) => (
                <button
                  ...
                  className={`w-full max-w-[1px] p-[3px] mt-[10px] h-full max-h-[1px] rounded-[50%] hover:border-transparent focus:outline-none focus:border-transparent ${
                    index === currentIndex ? "bg-[#83838f] border border-[#83838f]" : "bg-[#d7d7d7] border border-[#efefef]"
                  }`}
                />
              ))}
            </div>
            ...
            <div className="my-10 flex flex-col justify-center items-center">
                ...
                <>
                  ...
                  <div className="w-full flex flex-wrap justify-center gap-10 my-6 ">
                    {shopByCategoryData.map((item: any, index: number) => (
                      <ShopByCategoryCard key={`${item.id}-${index}`} product={item} adminSettingCardConfigure={adminSettingCardConfigure} />
                    ))}
                  </div>
                </>
              )}
              ...
              <img src={KnockoutOffers} alt="KnockoutOffers" className="cursor-pointer" />
            </div>
          </div>
        </div>
      </div>
