...
const SimilarProduct = ({
  ...
  const sameCategoryProducts = similarProducts.filter((item) => item.sub_category_type.sub_category.id === sub_category_id && item.id !== product_id);
  ...
  return (
    <div className="">
      ...
      <div className="flex lg:hidden flex-wrap gap-[27px] text-start">
        <Swiper modules={[Navigation, A11y]} spaceBetween={16} slidesPerView={1} centeredSlides={true} navigation>
          {sameCategoryProducts.slice(0, 10).map((product) => {
            const discountedPrice = Math.round(Number(product.price.toString().replace(/[^\d]/g, "")) * (1 - product.discount / 100));
