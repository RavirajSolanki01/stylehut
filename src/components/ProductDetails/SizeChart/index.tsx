...
import { BottomWearSizeChart, FootWearSizeChart, TopWearSizeChart } from "../../../assets";
...
const SizeChart: React.FC<ISizeChartProps> = ({
  ...
  const sizes = sizesData.map((item) => ({
    ...
    measurements:
      item.size_data?.size_chart_data?.map((chart: { size_field_name: any; size_field_value: string }) => ({
  ...
  const measurements =
    sizesData[0]?.size_data?.size_chart_data?.map((m: { size_field_name: any }) => ({
      name: m.size_field_name,
      unit: !sizesData[0]?.size_data?.is_cm ? "Inches" : "Cm",
    })) || [];

  const [measurementUnit, setMeasurementUnit] = useState<string>(measurements[0]?.unit ?? "Inches");
  ...
  const discountedPrice = Math.round(Number(price.toString().replace(/[^\d]/g, "")) * (1 - discount / 100));
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex justify-end">
      <aside className="w-full max-w-[700px] bg-white shadow-lg overflow-y-auto">
        ...
        <div className="flex items-start space-x-8 px-4 mt-2 mb-8">
          <img src={images?.[0] || ""} alt="Product" className="w-40 h-50 object-cover" />
          <div>
            ...
            <p className="mt-1 text-base font-semibold text-[#282c3f]">
              ₹{discountedPrice} <span className="line-through text-[#94969f] opacity-80 text-base mx-1">₹{price}</span>
              <span className="text-[#ff905a] font-semibold text-base"> ({discount}% OFF)</span>
            </p>
          </div>
        </div>
        <div className="relative">
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`flex-1 text-center py-3 h-fit cursor-pointer text-lg ${
                  activeTab === tab.id ? "text-[#3880FF] border-b-2 border-[#3880FF] font-semibold" : "text-black"
                }`}
              >
                {tab.label}
              </button>
          ...
          {activeTab === "size" ? (
            <div>
              <div className="px-4 py-2 flex items-center justify-end">
                <div className="border border-[#eaeaec] rounded-full p-[2px] flex items-center">
                  <button
                    ...
                    className={`cursor-pointer px-2 py-1 ${measurementUnit == "Inches" ? "text-white bg-black  rounded-full" : ""}`}
                  >
                    in
                  </button>
                  <button
                    onClick={() => setMeasurementUnit("Cm")}
                    className={`cursor-pointer px-2 py-1 ${measurementUnit == "Cm" ? "text-white bg-black rounded-full" : ""}`}
                  >
                    cm
                  </button>
                </div>
                    ...
                    <table className="w-full text-center border-collapse">
                      <thead>
                        <tr className="border border-[#eaeaec]">
                          ...
                          <th className="sizeChartWeb-newCell text-base font-[400] sticky left-[40px] bg-white z-20">Size</th>
                          {measurements.map((measurement: { name: any }) => (
                            <th className="sizeChartWeb-newCell text-base font-[400]" key={String(measurement.name)}>
                      ...
                      <tbody>
                        {sizes.map((size: any) => (
                          <tr className={`border border-[#eaeaec] ${size.available ? "" : "opacity-40 pointer-events-none"}`} key={size.skuId}>
                            ...
                            <td className={`sizeChartWeb-newCell text-base sticky left-[40px] bg-white z-10 ${size.available ? "" : "line-through"}`}>
                              {size.label}
                            </td>
                            {size.measurements.map((measurement: { minValue: any }, index: React.Key) => (
                              <td className={`sizeChartWeb-newCell text-base ${size.available ? "" : "line-through"}`} key={index}>
                                {inchesToCentimeters(measurementUnit, Number(measurement.minValue))}
                              </td>
                            ))}
                          </tr>
                        ))}
              ...
              <div className="p-4 flex items-center gap-3 sticky bottom-0 bg-white  shadow-[0_0px_24px_[#eaeaec]] z-[10]">
                {isAddedToCart ? (
                  <button
                  ...
                  <button
                    ...
                    className={`cursor-pointer ${
                      selectedSize ? "bg-[#3880FF] hover:bg-[#3880FF]" : "bg-gray-300 cursor-not-allowed"
                ...
                >
                  {isWishlisted ? (
                    <Favorite fill="bg-[#3880FF]" className="text-[#3880FF] " style={{ fontSize: "1rem" }} />
                  ) : (
                    <FavoriteBorderIcon className="!w-[20px] !h-[20px]" style={{ fontSize: "1rem" }} />
                  )}
                  <span>WISHLIST</span>
                </button>
              </div>
            ...
            <div className="transition-transform duration-300 ease-out pt-4 translate-y-0">
              <div className="text-center text-sm mt-2">* Garment Measurements in {measurementUnit}</div>
              <div className="text-center mt-4">
                <div className="px-4 text-[#282c3f] text-left font-semibold">How to measure yourself</div>
                <img src={renderImage()} alt="Size Representation" className="max-w-full w-auto h-auto mt-4 mx-auto mb-3  rounded-[4px]" />
              </div>
            </div>
          )}
        </div>
